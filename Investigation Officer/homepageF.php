    ...
    <div class="row flex-nowrap">
        ...
        <table class="table table-bordered">
            ...
            <tbody>
             <?php
                session_start();
                // Get officerId from the session
                $officerId = $_SESSION['officerId'];

                // Query to fetch complaints based on officerId
                $sql = "SELECT * FROM `currentcomplaints` WHERE officerId = $officerId";
                $result = mysqli_query($con, $sql);

                if ($result) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        // Your existing code to display complaints goes here
                        $id = $row['complaintId'];
